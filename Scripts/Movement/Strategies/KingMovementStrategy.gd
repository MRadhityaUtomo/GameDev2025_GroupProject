class_name KingMovementStrategy
...
func can_execute() -> bool:
	if not manager or not manager.player_body: 
		printerr("KingMovementStrategy: Manager atau player_body tidak ditemukan!")
		return false
	if _current_input_vector == Vector2.ZERO: return false

	if not manager.dynamic_tiles_node:
		printerr("KingMovementStrategy: manager.dynamic_tiles_node is null!")
		return false
...
func exit():
	...
	if manager: 
		manager.notify_movement_finished("king_move")
